    ...
    return (
        <div className='flex flex-col items-center w-full h-full md:w-5/6 gap-5 overflow-y-auto drain-scrollbar mt-6'>
            <div className='flex justify-between items-center gap-3 md:gap-0 w-full flex-col md:flex-row'>
                ...
                <Button className='flex items-center gap-2 w-56 py-2 rounded-md transition-colors duration-150' onClick={createPasskey}>
            ...
            <div className='flex flex-col gap-2 w-full'>
                {authManager.passkeys.length < 1
                    ? <p className='text-center text-muted-foreground text-sm py-4'>you have no passkeys. <span className='underline cursor-pointer' onClick={createPasskey}>create one!</span></p>
                    : authManager.passkeys.map((passkey) => (
                        <div key={passkey.id} className='flex items-center justify-between w-full py-3 px-4 border rounded-md gap-4'>
                            <span className='font-medium truncate'>{passkey.name}</span>

                            <span className='text-xs text-muted-foreground'>{passkey.lastUsed === 'never' ? 'never used' : 'last used ' + passkey.lastUsed}</span>
